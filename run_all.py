...
print(f"Step 1: Calculating ALL features with session length {session_length}s...", flush=True)
os.system(f"python pkl_to_mat.py --session-length {session_length} --remove-band {remove_band} --feature {feature}")
check_file_exists("data.mat", "Step 1")

if n_vids == 24:
    print("Using 24 videos for binary classification.", flush=True)
...
print(f"Step 2: Performing running normalization with {n_vids} videos, session length {session_length}s...", flush=True)
os.system(f"python running_norm.py --n-vids {n_vids} --session-length {session_length} --feature {feature} --remove-band {remove_band}")
...
print(f"Step 3: Smoothing data with LDS for {n_vids} videos, session length {session_length}s...", flush=True)
os.system(f"python smooth_lds.py --n-vids {n_vids} --session-length {session_length}")
check_file_exists(f"./smooth_{n_vids}", "Step 3")

# Step 4: Using SVM for Classification
print(f"Step 4: Training SVM classifier with {n_vids} videos, session length {session_length}s...", flush=True)
os.system(f"python main_de_svm.py --subjects-type {subject_type} --valid-method 10-folds --n-vids {n_vids} --session-length {session_length} --remove-band {remove_band}")

# Step 5: Evaluating Accuracy
print("Step 5: Testing SVM classifier...", flush=True)
os.system(f"python main_de_svm.py --train-or-test test --subjects-type {subject_type} --n-vids {n_vids} --session-length {session_length} --remove-band {remove_band}")

# Step 6: Plotting Results
print("Step 6: Plotting results...", flush=True)
os.system(f"python plot_result.py --subject-type {subject_type} --n-vids {n_vids} --session-length {session_length} --remove-band {remove_band}")
