...
# Remove hardcoded sec (will be passed as parameter)
# sec = 10

def load_srt_raw_newPre(data_dir, timeLen, timeStep, fs, channel_norm, time_norm, label_type, sec=10):
    n_channs = 30
    n_points = 7500
    data_len = fs * timeLen
...
def load_srt_de(data, channel_norm, isFilt, filtLen, label_type, sec=10):
    n_subs = 123
    if label_type =='cls2':
        n_vids = 24
    elif label_type =='cls9':
        n_vids = 28
    n_samples = np.ones(n_vids).astype(np.int32) * sec
...
def load_srt_pretrainFeat(datadir, channel_norm, timeLen, timeStep, isFilt, filtLen, label_type, sec=10):
    if label_type == 'cls9':
        n_samples = np.ones(28).astype(np.int32) * sec
    elif label_type == 'cls2':
        n_samples = np.ones(24).astype(np.int32) * sec
