...
parser.add_argument('--kernel', default='linear', type=str, help='SVM kernel type (linear, rbf)')
parser.add_argument('--search-method', default='grid', type=str, help='Parameter search method')
...
for fold in folds_list:
    ...
    data, label_repeat, n_samples = load_srt_de(data, channel_norm, isFilt, filtLen, label_type, sec=sec)
    ...
    if subjects_type == 'cross':
        ...
        # Calculate total number of segments per video
        segments_per_video = sec * n_vids
        
        # Calculate segments per fold (validation set size)
        segments_per_fold = segments_per_video / n_folds
        
        # Calculate start and end indices for this fold's validation set
        fold_start_idx = int(fold * segments_per_fold)
        fold_end_idx = int((fold + 1) * segments_per_fold)
        
        # Make sure indices don't exceed data length
        fold_end_idx = min(fold_end_idx, len(label_repeat))
        
        # Generate validation indices for this fold
        val_list = np.arange(fold_start_idx, fold_end_idx)
        
        # Generate the training indices (everything except validation indices)
        ...
        data_train = data[:, train_list, :].reshape(-1, data.shape[-1])
        data_val = data[:, val_list, :].reshape(-1, data.shape[-1])
